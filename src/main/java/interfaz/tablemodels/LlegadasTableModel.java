...
public class LlegadasTableModel extends AbstractTableModel {
    ...
    public LlegadasTableModel(List<VueloDiario> listaVuelos) {
        Collections.sort(listaVuelos, Comparator.comparing(VueloDiario::getHoraLlegadaReal));
    ...
    public Object getValueAt(int rowIndex, int columnIndex) {
        ...
        return switch (columnIndex) {
            case 0 -> vueloDiario.getCodVuelo();
            case 1 -> formatoFecha.format(vueloDiario.getFechaVuelo());
            case 2 -> formatoHora.format(vueloDiario.getHoraSalidaReal());
            case 3 -> formatoHora.format(vueloDiario.getHoraLlegadaReal());
            case 4 -> vueloDiario.getNumPlazasOcupadas();
            case 5 -> String.format("%.0f€", vueloDiario.getPrecio());
            default -> null;
        };
    }
    ...
    public void filtrarPorAeropuerto(Aeropuerto aeropuertoBase) {
        if (aeropuertoBase != null) {
            Set<String> codigosVueloValidos = listaVuelos.stream()
                    .filter(vuelo -> vuelo.getCodIataDestino().equals(aeropuertoBase.getCodIata()))
        ...
        // Luego de filtrar por aeropuerto, reiniciar el filtro por fecha
